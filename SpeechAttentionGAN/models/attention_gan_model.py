...
class AttentionGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            self.model_names = (
                ["G_A", "G_B", "D_A", "D_B", "D2_A", "D2_B"]
                if opt.use_cycled_discriminators
                else ["G_A", "G_B", "D_A", "D_B"]
            )
        ...
        if self.isTrain:  # define discriminators
            ...
            if opt.use_cycled_discriminators:
                self.netD2_A = networks.define_D(
                    opt.input_nc,
                    opt.ndf,
                    opt.netD,
                    opt.n_layers_D,
                ...
                self.netD2_B = networks.define_D(
                    opt.input_nc,
                    opt.ndf,
                    opt.netD,
                    opt.n_layers_D,
        ...
        if self.isTrain:
            ...
            if opt.use_cycled_discriminators:
                self.rec_A_pool = ImagePool(
                    opt.pool_size
                )  # create image buffer to store previously generated images
                self.rec_B_pool = ImagePool(
                    opt.pool_size
                )  # create image buffer to store previously generated images
            ...
            disc_parameters = (
                itertools.chain(
                    ...
                    self.netD2_A.parameters(),
                    self.netD2_B.parameters(),
                )
                if opt.use_cycled_discriminators
                else itertools.chain(self.netD_A.parameters(), self.netD_B.parameters())
            )
            self.optimizer_D = torch.optim.Adam(
                disc_parameters, lr=opt.lr, betas=(opt.beta1, 0.999)
            )
    ...
    def forward(self):
        ...
        if not self.opt.use_mask:
            ...
            (
                ...
                self.fake_B,
                torch.ones(
                    self.fake_B.size(0), 1, self.fake_B.size(2), self.fake_B.size(3)
                ),
            )  # G_B(G_A(A))
            (
                self.fake_A,
            ...
            (
                ...
                self.fake_A,
                torch.ones(
                    self.fake_A.size(0), 1, self.fake_A.size(2), self.fake_A.size(3)
                ),
            )  # G_A(G_B(B))
        ...
        if lambda_idt > 0:
            ...
            if self.opt.use_mask:
                (
                    ...
                    self.real_B,
                    torch.ones(
                        self.real_B.size(0), 1, self.real_B.size(2), self.real_B.size(3)
                    ),
                )
                self.loss_idt_A = (
