...
def get_inception_probs(batch_size, images, inception_images, logits):
    ...
    for i in range(n_batches):
        ...
        preds[i * batch_size : (i + 1) * batch_size] = logits.eval(
...
def get_inception_score(batch_size, images, inception_images, logits, splits=10):
    ...
    assert (
        np.min(images[0]) >= 0 and np.max(images[0]) > 10
    ), "Image values should be in the range [0, 255]"
    print(
        "Calculating Inception Score with %i images in %i splits"
    ...
    return (
        mean,
        std,
    )  # Reference values: 11.34 for 49984 CIFAR-10 training set images, or mean=11.31, std=0.08 if in 10 splits.
