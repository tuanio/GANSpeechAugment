...
def apsipa_exp(
    ...
    csv_path,
    sources,
    data_cache="/content/AttentionGAN-VC/data_cache",
    results_dir="/content/AttentionGAN-VC/results",
    ...
    for name, source in zip(names, sources):
        ...
        info[
            "comment"
        ] = f"AttentionGAN-VC trained for 200 epochs+ 200 with decay on {source} dataset"
        log(csv_path, info)
...
def timit_exp1(
    ...
    for name, noise_dB in zip(names, noise_dBs):
        ...
        run(
            f"python test.py --dataroot data_cache --name {name} --model attention_gan --dataset_mode audio --norm instance --phase test --no_dropout --load_size_h 128 --load_size_w 128 --crop_size 128 --batch_size 1 --gpu_ids 0 --input_nc 1 --output_nc 1 --use_mask --checkpoints_dir /content/drive/MyDrive/TASLP/EXP1/checkpoints"
        )
        avg_lsd, std_lsd = lsd(
            os.path.join(data_cache, "noisy", "test"),
            os.path.join(results_dir, name, "test_latest", "audios", "fake_B"),
            use_gender=False,
        )
        avg_mssl, std_mssl = mssl(
            os.path.join(data_cache, "noisy", "test"),
        ...
        log(
            csv_path,
            name,
            f"Training {name} for EXP1 with SNR {noise_dB} for 200 epochs with cycled_disc, WITHOUT phase and mask. LambdaA & B 10 , lambda_identity 0.5",
            avg_lsd,
            std_lsd,
            avg_mssl,
            std_mssl,
...
if __name__ == "__main__":
    ...
    apsipa_exp(
        [f"AttentionGAN_nophase_mask_cycdisc_{i}" for i in ["np_rats"]],
        csv_path,
        sources,
    )
