            ...
            if (i + 1) % self.sample_step == 0:
                with torch.no_grad():
                    ...
                    attention_path = os.path.join(
                        self.sample_dir, "{}-attention.jpg".format(i + 1)
                    )
                    content_path = os.path.join(
                        self.sample_dir, "{}-content.jpg".format(i + 1)
                    )
                    save_image(
                        self.denorm(x_concat.data.cpu()), sample_path, nrow=1, padding=0
                    )
                    save_image(
                        self.denorm(attention_concat.data.cpu()),
                        attention_path,
                        nrow=1,
                        padding=0,
                    )
                    save_image(
                        self.denorm(content_concat.data.cpu()),
                        content_path,
                        nrow=1,
                        padding=0,
                    )
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_real, c_org) in enumerate(data_loader):
                ...
                attention_path = os.path.join(
                    self.result_dir, "{}-attention.jpg".format(i + 1)
                )
                save_image(
                    self.denorm(attention_concat.data.cpu()),
                    attention_path,
                ...
                content_path = os.path.join(
                    self.result_dir, "{}-content.jpg".format(i + 1)
                )
                save_image(
                    self.denorm(content_concat.data.cpu()),
                    content_path,
