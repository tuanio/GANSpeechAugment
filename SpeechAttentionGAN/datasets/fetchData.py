...
def transfer_timit(
    ...
    for clip in clean_clips_train_all:
        ...
        if (
            librosa.get_duration(filename=clip) + clean_train_duration_saved
            < train_duration_max
            and librosa.get_duration(filename=clip) > 1
        ):
    ...
    for clip in noisy_clips_train_all:
        basename = os.path.basename(clip)
        if (
            librosa.get_duration(filename=clip) + noisy_train_duration_saved
            < train_duration_max
            and librosa.get_duration(filename=clip) > 1
        ):
    ...
    for clip in val_clips_all:
        ...
        if (
            librosa.get_duration(
                filename=os.path.join(timit_dir, "test", "clean", clip)
            )
            + val_duration_saved
            < val_duration_max
