...
def save_audio(opt, visuals_list, img_path, use_phase=False, label="fakeA"):
    ...
    for visual in visuals_list:
        ...
        if use_phase:
            ...
            im_mag = im_mag.resize(
                (defaults["fix_w"], defaults["n_fft"] // 2 + 1), Image.LANCZOS
            )
            im_phase = im_phase.resize(
                (defaults["fix_w"], defaults["n_fft"] // 2 + 1), Image.LANCZOS
            )
            ...
            if flag_first:
                ...
                mag_spec = np.concatenate(
                    (mag_spec, im_mag), axis=1
                )  # concatenating specs to obtain original.
                phase_spec = np.concatenate(
                    (phase_spec, im_phase), axis=1
                )  # concatenating specs to obtain original.
        else:
