...
for i, batch in enumerate(dataloader):
    ...
    save_image(
        ...
        "%s/%s/%04d_real_A.png" % (opt.save_name, "testing", i + 1),
    )
    save_image(
        real_B.data.cpu()[0] * 0.5 + 0.5,
        "%s/%s/%04d_real_B.png" % (opt.save_name, "testing", i + 1),
    )
    ...
    save_image(
        mask_A.data.cpu()[0],
        "%s/%s/%04d_mask_A.png" % (opt.save_name, "testing", i + 1),
    )
    save_image(
        mask_B.data.cpu()[0],
        "%s/%s/%04d_mask_B.png" % (opt.save_name, "testing", i + 1),
    )
