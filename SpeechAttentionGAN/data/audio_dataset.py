...
class AudioDataset(BaseDataset):
    ...
    def modify_commandline_options(parser, is_train):
        ...
        parser.set_defaults(
            preprocess="resize",
            load_size_h=defaults["load_size_h"],
            load_size_w=defaults["load_size_w"],
            crop_size=min(defaults["load_size_h"], defaults["load_size_w"]),
        )  # TODO Change crop to h and w
    ...
    def __init__(self, opt):
        ...
        self.clean_specs = Parallel(n_jobs=self.num_cores, prefer="threads")(
            delayed(processInput)(
                i, self.spec_power, self.phase, self.channels, self.opt.use_phase
            )
    ...
    def __getitem__(self, index):
        ...
        if self.opt.use_phase:
            ...
            A = torch.cat(
                [A_transform(A_img[0]), A_transform(A_img[1])], dim=0
            )  # Appending magnitude and phase components along the channel dimension
        else:
        ...
        if (
            self.data_load_order == "aligned" or self.opt.serial_batches
        ...
        if self.opt.use_phase:
            ...
            B = torch.cat(
                [B_transform(B_img[0]), B_transform(B_img[1])], dim=0
            )  # Appending magnitude and phase components along the channel dimension
        else:
