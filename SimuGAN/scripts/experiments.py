...
def validate(name, epochs, data_cache, results_dir):
    ...
    for epoch in epochs:
        run(
            f"python test.py --split val --dataroot data_cache --name {name} --CUT_mode CUT --phase train --state Test --checkpoints_dir /content/drive/MyDrive/APSIPA/Results/checkpoints --results_dir {results_dir} --epoch {epoch}"
        )
        avg_lsd, std_lsd = lsd(
...
def apsipa_exp(
    ...
    csv_path,
    sources,
    data_cache="/content/CUTGAN/data_cache",
    results_dir="/content/CUTGAN/results",
    ...
    for name, source in zip(names, sources):
        ...
        run(
            f"python train.py --split train --dataroot data_cache --name {name} --CUT_mode CUT --display_id 0 --state Train --checkpoints_dir /content/drive/MyDrive/APSIPA/Results/checkpoints --no_html"
        )
        ...
        for metric in ["min_lsd_epoch", "min_mssl_epoch"]:
            epoch = info[metric]
            run(
                f"python test.py --split test --dataroot data_cache --name {name} --CUT_mode CUT --phase train --state Test --checkpoints_dir /content/drive/MyDrive/APSIPA/Results/checkpoints --results_dir {results_dir} --epoch {epoch}"
            )
        ...
        info[
            "comment"
        ] = f"CUTGAN trained for 200 epochs+ 200 with decay on {source} dataset."
        log(csv_path, info)
